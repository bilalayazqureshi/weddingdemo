...
class WeddingEventWebControllerHtmlUnitTest {
	...
	void testEditNonExistentEvent() throws Exception {
		when(weddingEventService.getEventById(1L)).thenReturn(null);
	...
	void testEditExistentEvent() throws Exception {
		...
		when(weddingEventService.getEventById(1L)).thenReturn(original);
		...
		verify(weddingEventService).updateEventById(1L,
	...
	void testEditNewEvent() throws Exception {
		...
		verify(weddingEventService)
				.insertNewEvent(new WeddingEvent(null, "NewName", LocalDate.of(2025, 12, 24), "Florence"));
	}

	@Test
	void testDeleteEvent_ShouldDisplayConfirmationMessage() throws Exception {
		doNothing().when(weddingEventService).deleteEventById(3L);

		HtmlPage page = webClient.getPage("/delete/3");

		// verify service call
		verify(weddingEventService, times(1)).deleteEventById(3L);
